...
class Normalize(object):
    ...
    def __call__(self, results):
        ...
        # for key in results.get('img_fields', ['img']):
        #     print(self.mean.shape, self.std.shape, results[key].shape)
...
def get_corners(bboxes):
    ...
    x4 = x1 + width#bboxes[:,2].reshape(-1,1)
    y4 = y1 + height#bboxes[:,3].reshape(-1,1)
...
def rotate_annotation(origin, annotation, degree):
    ...
    r_w = round(max(x_coords) - min(x_coords))
    r_h = round(max(y_coords) - min(y_coords))
...
class RandomRotate(object):
    ...
    def __init__(self, rotation_range = (-90, 90)):
    ...
    def __call__(self, results):
        w,h,cx,cy = None, None, None, None
        angle = np.random.randint(self.rotation_range[0], self.rotation_range[1])

        for key in results.get('img_fields', ['img']):
            img = results[key]
            rot_img = rotate_image(img, angle )
            img =  crop_to_center(img, rot_img)
            results[key] = np.array(img, dtype = np.float32)
            img_shape = img.shape
        valid_flag = False
        # # crop bboxes accordingly and clip to the image boundary
        for key in results.get('bbox_fields', []):
            ...
            if(len(bboxes) != 0):
                ...
                for bbox in bboxes:
                    ...
                    coor[0] += coor[2]/2
                    coor[1] += coor[3]/2
                    coor = np.array(rotate_annotation((img.shape[1] / 2, img.shape[0] / 2), coor, angle), dtype = np.int)
                    coor[0] -= coor[2]/2
                    coor[1] -= coor[3]/2
                    x_center = coor[0] + coor[2]/2
                    y_center = coor[1] + coor[3]/2
                    new_bbox.append([x_center - 25, y_center - 25, x_center +25, y_center + 25])
                    # new_bbox.append([coor[0], coor[1], coor[2] + coor[0], coor[3] + coor[1]])
                    
                bboxes = np.array(new_bbox, dtype = np.float32)
            
            bboxes[:, 0::2] = np.clip(bboxes[:, 0::2], 0, img_shape[1])
            bboxes[:, 1::2] = np.clip(bboxes[:, 1::2], 0, img_shape[0])
            valid_inds = (bboxes[:, 2] > bboxes[:, 0]) & (
                bboxes[:, 3] > bboxes[:, 1])
            # if(False in valid_inds):
            #     print("RRRR")
            #     print(a)
            #     print(bboxes, angle)
            # When there is no gt bbox, cropping is conducted.
...
class RandomReplace(object):
    """Random Replace the image & bboxes & masks.
    ...
    def __init__(self, dataset_path = None, replace_prob = 0.3):
        self.dataset_path = dataset_path
        self.replace_prob = replace_prob
        import os
        self.classes = ['Mitosis', 'Mitosislike', 'Granulocytes', 'Tumorcells']
        # The key correspondence from bboxes to labels and masks.
        self.bbox2label = {
            'gt_bboxes': 'gt_labels',
            'gt_bboxes_ignore': 'gt_labels_ignore'
    ...
    def __call__(self, results):
        ...
        hue, sat, br = mmcv.bgr2hsv(img[..., :: -1]).mean(axis = (0, 1))
        ...
        for key in results.get('bbox_fields', []):
            ...
            if(len(bboxes) != 0):
                for bbox in bboxes:
                    xmin, ymin, xmax, ymax = bbox
                    sampled_class = self.classes[np.random.choice(4, 1, p=[0.8, 0.2/3, 0.2/3, 0.2/3])[0]]
                    area = (ymax - ymin) * (xmax - xmin)
                    if(np.random.rand() > 1 - self.replace_prob and area == 2500):
                        # print("in")
                        full_path = '{}/{}/'.format(self.dataset_path,sampled_class)
                        # print(full_path)

                        # cropped_img
                        selected_list = os.listdir(full_path)
                        selected_data = selected_list[np.random.choice(len(sampled_class), 1 )[0]]
                        selected_data_path = '{}{}'.format(full_path, selected_data)
                        # print("----->", selected_data_path)
                        selected_image = cv2.imread(selected_data_path)
                        selected_image = np.array(selected_image, dtype = np.float32)

                        selected_image = mmcv.bgr2hsv(selected_image)
                        t_hue, t_sat, t_br = selected_image.mean(axis = (0, 1))
                        # print(t_hue, hue)
                        selected_image[..., 0] += hue - t_hue
                        selected_image[..., 0][selected_image[..., 0] > 360] -= 360
                        selected_image[..., 0][selected_image[..., 0] < 0] += 360
                        selected_image[..., 1] += sat - t_sat
                        selected_image[..., 2] += br - t_br
                        selected_image[selected_image < 0] = 0
                        selected_image = mmcv.hsv2bgr(selected_image)
                        selected_image = np.array(selected_image, dtype = np.float32)[..., ::-1]
                        
                        img[int(ymin) : int(ymax), int(xmin) : int(xmax)] = selected_image
                        # cv2.imwrite('x.png', img)
                        if(sampled_class == 'Mitosis'):
                            new_bbox.append([xmin, ymin, xmax, ymax])
                        else:
                            new_bbox.append([-1, -1, -1, -1])
                    else:
                        new_bbox.append([xmin, ymin, xmax, ymax])
                if(len(new_bbox) != 0):
                    bboxes = np.array(new_bbox, dtype = np.float32)
        ...
        # cv2.imwrite('t/{}.png'.format(np.random.randint(0, 1e6)), img)
        for key in results.get('img_fields', ['img']):
            results[key] = np.array(img, dtype = np.float32)
    ...
    def __repr__(self):
        return self.__class__.__name__ + f'(rand_replace={self.crop_size})'
